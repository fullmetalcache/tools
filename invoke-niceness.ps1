...
[CmdletBinding( DefaultParameterSetName = 'RunLocal', SupportsShouldProcess = $True , ConfirmImpact = 'High')] Param (
    ...
    function Local:Inject-RemoteNiceness ([Int] $ProcessID)
    {
        ...
        if ($64bitOS) 
        {
            ...
            {
                Throw 'Niceness injection targeting a 64-bit process from 32-bit PowerShell is not supported. Use the 64-bit version of Powershell if you want this to work.'
            }
            elseif ($IsWow64) # 32-bit Wow64 process
            {
                ...
                Write-Verbose 'Using 32-bit Niceness.'
            }
            else # 64-bit process
            {
                ...
                $Niceness = $Niceness64
                Write-Verbose 'Using 64-bit Niceness.'
            }
        }
        else # 32-bit CPU
        {
        ...
        Write-Verbose "Niceness memory reserved at 0x$($RemoteMemAddr.ToString("X$([IntPtr]::Size*2)"))"
        ...
        Write-Verbose 'Niceness injection complete!'
    }

    function Local:Inject-LocalNiceness
    {
        if ($PowerShell32bit) {
            if ($Niceness32.Length -eq 0)
            {
        ...
        Write-Verbose "Niceness memory reserved at 0x$($BaseAddress.ToString("X$([IntPtr]::Size*2)"))"
    ...
    if ( $PSBoundParameters['ProcessID'] )
    {
        ...
        Write-Verbose "Injecting Niceness into PID: $ProcessId"
        
        if ( $Force -or $psCmdlet.ShouldContinue( 'Do you wish to carry out your evil plans?',
                 "Injecting Niceness injecting into $((Get-Process -Id $ProcessId).ProcessName) ($ProcessId)!" ) )
        {
            Inject-RemoteNiceness $ProcessId
        }
    }
    else
    {
        ...
        Write-Verbose "Injecting Niceness into PowerShell"
        
        if ( $Force -or $psCmdlet.ShouldContinue( 'Do you wish to carry out your evil plans?',
                 "Injecting Niceness into the running PowerShell process!" ) )
        {
            Inject-LocalNiceness
        }
    }   
}
